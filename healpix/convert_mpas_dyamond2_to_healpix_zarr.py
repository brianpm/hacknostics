...
def main():
    ...
    oloc = Path("/glade/derecho/scratch/digital-earths-hackathon/mpas_DYAMOND2")
    ...
    dataloc = Path("/glade/campaign/mmm/wmr/fjudt/projects/dyamond_2/3.75km")
    datafils = sorted(dataloc.glob("history.2020*.nc")) # note: 62GB per file
    ...
    out_prefix = "DYAMOND2_history"

    overwrite_weights = False

    overwrite_zarr = False

    do_pre_load = False # flag to load entire file into memory
    
    do_separate_variables = True # flag to write each variable into its own zarr

    # Add tracking file path
    tracking_file = oloc / f"{out_prefix}_processed_files_vars.txt"
    ...
    for i, fil in enumerate(datafils):
        ...
        if hasattr(data, "compute") and do_pre_load:
        ...
        if do_separate_variables:
            # Iterate through variables
            for var_name in data.data_vars:
                logger.info(f"Processing variable: {var_name}")
                load_start_time = time.perf_counter()
                data_var = data[[var_name]].compute()
                load_end_time = time.perf_counter()
                logging.info(f"[VAR TIMER (1)] DATA LOADING TIME {var_name}: {load_end_time - load_start_time} seconds.")
                dsout = remap_mpas_to_hp(data_var, eweights, evweights, order)
                if hasattr(dsout, "compute"):
                    load_start_time = time.perf_counter()
                    dsout = dsout.compute()
                    load_end_time = time.perf_counter()
                    logging.info(f"[VAR TIMER (2)] DATA LOADING TIME {var_name}: {load_end_time - load_start_time} seconds.")
                # save highest resolution output
                fn = oloc / f"{out_prefix}_{var_name}_to_hp{order}.zarr"
                save_to_zarr(dsout, fn, clobber=overwrite_zarr)
                # coarsen and save zarr
                mpas_hp_to_zarr(dsout, order, oloc, f"{out_prefix}_{var_name}", clobber=overwrite_zarr)
        else:
            dsout = remap_mpas_to_hp(data, eweights, evweights, order)

            if hasattr(dsout, "compute") and do_pre_load:
                load_start_time = time.perf_counter()
                dsout = dsout.compute()
                load_end_time = time.perf_counter()
...
def pre_proc_mpas_file(datafil, meshfil):
    ...
    if hasattr(ds_mpas, "compute"):
        load_start_time = time.perf_counter()
        ds_mpas = ds_mpas.compute()
        load_end_time = time.perf_counter()
        logging.info(f"[TIMER (000)] DATA LOADING TIME: {load_end_time - load_start_time} seconds.")
