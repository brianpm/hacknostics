...
def main():
    ...
    datafils = sorted(dataloc.glob("EW_B2000_CAM7_15km_58L_2D_3hourly.000#.nc")) # note: 4.4GB per file
...
def mask_and_coarsen(maskc,damask,da):
    #da - data field
    #damask - mask on the data fields zoom level
    #maskc - mask on the coarser zoom level
    temvar1 = da*damask
...
def mpas_hp_to_zarr(ds, zoom, outloc, zarr_name_prefix, clobber=None):
    ...
    #DD get masking fields for weighted interpolation of soil moisture (mrso) and seaice (siconc)
    mfile = f"hist_postproc_healpix/2D_constant/EW_B2000_CAM7_15km_58L_2D_constant_fields_to_hp{zoom}.zarr"
    dc = xr.open_dataset(mfile, engine="zarr", chunks={})
    dn=ds.copy()
    for x in range(zoom-1,0,-1):
        fn = outloc / f"{zarr_name_prefix}_to_hp{x}.zarr"

        mfile = f"hist_postproc_healpix/2D_constant/EW_B2000_CAM7_15km_58L_2D_constant_fields_to_hp{x}.zarr"
        dcc = xr.open_dataset(mfile, engine="zarr", chunks={})
        #DD some variables coarsen directly, others coarsen with some sort of mask
        # coarsen by one level
        
        #DD this does direct coarsening
        dx = dn.coarsen(cell=4).mean()

        #DD other variables needing mask coarsening like land fraction
        dx["mrso"] = mask_and_coarsen(dcc.sftlf,dc.sftlf,dn.mrso).copy()
        ocfracx = 1 - dcc.sftlf
        ocfracn = 1 - dc.sftlf
        dx["siconc"] = mask_and_coarsen(ocfracx,ocfracn,dn.siconc).copy()
