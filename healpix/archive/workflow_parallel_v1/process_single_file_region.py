...
def process_file(nc_file_path, zarr_dir, zarr_prefix, time_offset, # num_time_steps will be 1
    ...
    try:
        ...
        for ivar, var_name_zarr in enumerate(variables_in_zarr):
            ...
            # Skip if the variable in Zarr doesn't have a time dimension (e.g. static vars from init)
            # although our current init creates all with time.
            if 'time' not in zarr_dims:
                logger.info(f"Variable '{var_name_zarr}' in Zarr store does not have a 'time' dimension. Skipping for time-slice write.")
                continue

            logger.info(f"  Processing variable: {var_name_zarr} ({ivar+1}/{number_of_variables})")
            ...
            # Transpose to final Zarr order: (time, healpix_dim_name_zarr, [actual_vert_dim_name_if_3d], ...)
            # This order must match how initialize_healpix_zarr.py created the Zarr arrays.
            ...
            # Append any other dimensions that were carried through (e.g., 'member' if it existed)
            # and are not already in the core Zarr order.
