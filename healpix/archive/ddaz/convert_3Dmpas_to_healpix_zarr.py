...
def main():
    ...
    # 1hr files
    #dataloc = Path("/glade/campaign/univ/ucsu0085/coupled_l58_15km/hist_postproc_qu/2D_hourly")
    #datafils = sorted(dataloc.glob("EW_B2000_CAM7_15km_58L_2D_hourly.0001-0*.nc")) 
    #oloc = Path("/glade/campaign/univ/ucsu0085/coupled_l58_15km/hist_postproc_healpix/2D_hourly")
    #out_prefix = "2D_hourly"

    # 3hr files
    #dataloc = Path("/glade/campaign/univ/ucsu0085/coupled_l58_15km/hist_postproc_qu/2D_3hourly")
    #datafils = sorted(dataloc.glob("EW_B2000_CAM7_15km_58L_2D_3hourly.*.nc")) # note: 4.4GB per file
    #oloc = Path("/glade/campaign/univ/ucsu0085/coupled_l58_15km/hist_postproc_healpix/2D_3hourly")
    #out_prefix = "2D_3hourly"

    # 6hr ncells files
    dataloc = Path("/glade/campaign/univ/ucsu0085/coupled_l58_15km/hist_postproc_qu/3D_6hourly_cell")
    datafils = sorted(dataloc.glob("EW_B2000_CAM7_15km_58L_3D_6hourly.000*.nc")) # note: 4.4GB per file
    oloc = Path("/glade/campaign/univ/ucsu0085/coupled_l58_15km/hist_postproc_healpix/3D_6hourly_cell")
    out_prefix = "3D_6hourly"
    ...
    #DD - read in the isobaric levels
    plev = ds_static["plev"]
    ...
    for i, fil in enumerate(datafils):
        ...
        #DD - assign the pressures to the lev dimension
        dsout = dsout.assign_coords(lev=plev)
...
def remap_mpas_to_hp(ds, cell_weights, vertex_weights, zoom):
    ...
    #DD create variable mask which is fraction of grid cell above the surface
    #DD This is necessary for proper coarsening. For instance a proper global mean
    #DD is sum(mask*var) / sum(mask)
    mask = xr.DataArray( data = np.where( np.isnan(c_vars.ta.values),0.,1.),
        coords = c_vars.ta.coords,  
        dims = c_vars.ta.dims,
        attrs=dict(
            long_name="above ground fraction",
            units="1",
            )
        )
    dx = c_vars.copy()
    c_vars = dx.assign(mask=mask)
    #DD end new mask code
...
#DD New function to properly coarsen a variable using mask
def mask_and_coarsen(maskc,damask,da):
    temvar1 = da*damask
    temvar2 = temvar1.coarsen(cell=4).sum()
    return xr.where(maskc>0.001, temvar2/(4*maskc), np.nan)
...
def mpas_hp_to_zarr(ds, zoom, outloc, zarr_name_prefix, clobber=None):
    ...
    for x in range(zoom-1,0,-1):
        ...
        #DD New code to coarsen mask, fraction of isobaric level above surface
        mask = dn.mask.coarsen(cell=4).mean()

        #DD this sets up the coarse dataset
        dx = dn.coarsen(cell=4).mean()

        #DD loop through all variables and coarsen var*mask
        #this needs to be converted to a loop
        # currently, this list obtained by ncdump -h on the mpas file
        dx["ta"] = mask_and_coarsen(mask,dn.mask,dn.ta).copy()
        dx["zg"] = mask_and_coarsen(mask,dn.mask,dn.zg).copy()
        dx["ua"] = mask_and_coarsen(mask,dn.mask,dn.ua).copy()
        dx["va"] = mask_and_coarsen(mask,dn.mask,dn.va).copy()
        dx["wa"] = mask_and_coarsen(mask,dn.mask,dn.wa).copy()
        dx["wap"] = mask_and_coarsen(mask,dn.mask,dn.wap).copy()
        dx["hur"] = mask_and_coarsen(mask,dn.mask,dn.hur).copy()
        dx["hus"] = mask_and_coarsen(mask,dn.mask,dn.hus).copy()
        dx["clw"] = mask_and_coarsen(mask,dn.mask,dn.clw).copy()
        dx["cli"] = mask_and_coarsen(mask,dn.mask,dn.cli).copy()
        dx["mfrain"] = mask_and_coarsen(mask,dn.mask,dn.mfrain).copy()
        dx["mfgrau"] = mask_and_coarsen(mask,dn.mask,dn.mfgrau).copy()
        dx["mfsnow"] = mask_and_coarsen(mask,dn.mask,dn.mfsnow).copy()

        #DD get the mask values copied into the dataset
        dx["mask"] = mask.copy()
        #DD end added code
